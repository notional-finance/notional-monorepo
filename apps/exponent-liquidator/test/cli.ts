  ...
  .action(async (suiteFile: string, testName: string, options: any) => {
    ...
    try {
      ...
      if (!testCase) {
        ...
        console.log(
          `Available tests: ${suite.testCases.map((tc) => tc.name).join(', ')}`
        );
        process.exit(1);
      }

      const forkBlock = options.forkBlock
        ? parseInt(options.forkBlock)
        : undefined;
...
program
  ...
  .option(
    '-b, --fork-block <number>',
    'Override fork block number for all tests'
  )
  ...
  .action(async (suiteFile: string) => {
    ...
    try {
      ...
      suite.testCases.forEach((testCase, index) => {
        ...
        console.log(
          `   ✅ Expected success: ${testCase.expectedResults.successfulTransactions}\\n`
        );
      });
    } catch (error) {
