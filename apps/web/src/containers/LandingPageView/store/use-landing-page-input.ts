...
export function useLandingPageInput() {
  ...
  const {
    ...
    fCashAmount,
    lendOrBorrow,
  } = useObservableState(landingState$, initialLandingState);
  ...
  const tradedRate =
    inputAmountUnderlying && fCashAmount
      ? selectedMarket?.interestRate(fCashAmount, inputAmountUnderlying)
      : undefined;
  const slippage =
    selectedMarket && tradedRate
      ? Market.formatInterestRate(
          Math.abs(selectedMarket.marketAnnualizedRate() - tradedRate)
        )
      : undefined;
  const maturity = selectedMarket
    ? formatMaturity(selectedMarket?.maturity)
    : '';
  const queryString =
    inputAmount && selectedMarketKey
      ? `?amount=${inputAmount.toString()}&marketKey=${selectedMarketKey}`
      : '';
  ...
  try {
    if (currencyId) {
      ...
      const nTokenIncentiveYield =
        NTokenValue.getNTokenIncentiveYield(currencyId);
      nTokenYieldString = Market.formatInterestRate(
        nTokenYield + nTokenIncentiveYield,
        2
      );
    }
  } catch (e) {
  ...
  useEffect(() => {
    ...
    if (!selectedMarketKey) {
      ...
      if (mostLiquidMarket)
        updateLandingState({ selectedMarketKey: mostLiquidMarket.marketKey });
    }
