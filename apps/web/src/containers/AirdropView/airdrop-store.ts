import { getInRemoteStorage, logError } from '@notional-finance/utils'
...
export const claims$ = combineLatest([selectAirdropState('address'), system$]).pipe(
  filter(([a, s]) => !!a && !!s),
...
const AIRDROP_FILE = 'https://notional-airdrop.storage.googleapis.com/AirdropMerkleTree.json'
const INCENTIVE_FILE = 'https://storage.googleapis.com/notional-airdrop/IncentiveAirdropTree.json'
...
async function getActiveClaims(userAddress: string, provider: ethers.providers.Provider) {
  try {
    // Will return the checksummed address for comparison
    const userAddressParsed = ethers.utils.getAddress(userAddress)

    const { isClaimed: isOriginalClaimed, userClaim: originalClaim } = await checkAirdropClaim(
      AIRDROP_FILE,
      AIRDROP_ADDRESS,
      userAddressParsed,
      provider
    ...
    if (originalClaim && !isOriginalClaimed) {
      ...
      const { isClaimed: isIncentiveClaimed, userClaim: incentiveClaim } = await checkAirdropClaim(
        INCENTIVE_FILE,
        INCENTIVE_ADDRESS,
        userAddressParsed,
        provider
