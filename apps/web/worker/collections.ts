...
async function getCollectionDefinition(collectionId: string, apiToken: string) {
  ...
  return await response.json<CollectionDefinition>();
}
...
export async function getAllVaults(apiToken: string) {
  ...
  const items = await fetchCollectionItems(VAULT_COLLECTION_ID, apiToken);
  const dereferenced = await dereferenceCollection(definition, items, apiToken);
