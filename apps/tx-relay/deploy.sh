...
gcloud --project monitoring-agents \
  ...
  --set-secrets=AUTH_TOKEN=projects/663932775145/secrets/TX_RELAY_AUTH_TOKEN/versions/latest,DD_API_KEY=projects/663932775145/secrets/DD_API_KEY/versions/latest
...
gcloud --project monitoring-agents \
  ...
  --max-instances=1 \
  --service-account=monitoring-agents@appspot.gserviceaccount.com \
  --set-secrets=DD_API_KEY=projects/663932775145/secrets/DD_API_KEY/versions/latest
...
echo "$cron_jobs" | while IFS='|' read -r description url schedule; do
  ...
  if
    gcloud scheduler jobs describe "$job_name" --location=us-central1 >/dev/null 2>&1
  then
    ACTION="update"
  else
    ACTION="create"
