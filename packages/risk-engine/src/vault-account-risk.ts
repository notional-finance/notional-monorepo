...
export class VaultAccountRiskProfile extends BaseRiskProfile {
  ...
  estimatedWithdrawTime(
    withdrawRequests?: { lastUpdateTimestamp: number; requestId: string }[]
  ) {
    return this.hasPendingWithdraw
      ? this.model
          ...
          .reduce(
            (m, t, i) =>
              m ||
              t.estimatedWithdrawTime(
                withdrawRequests ? withdrawRequests[i] : undefined
              ),
  ...
  maxWithdraw(_token: TokenDefinition = this.vaultShares.token) {
    ...
    if (this.hasFinalizedWithdraw) {
      ...
      feesPaid = result.feesPaid.toToken(netUnderlyingForVaultShares.token);
