...
export async function getMeshOptions(): Promise<GetMeshOptions> {
  ...
  const notionalV3Handler = new GraphqlHandler({
    ...
    config: {
      endpoint:
        'https://api.studio.thegraph.com/query/33671/notional-finance-v3-{context.chainName:arbitrum}/v0.0.102',
    },
    baseDir,
    cache,
    pubsub,
...
export type AllVaultsQuery = {
  vaultConfigurations: Array<
    Pick<VaultConfiguration, 'id' | 'vaultAddress' | 'strategy' | 'name'>
  >;
  _meta?: Maybe<{ block: Pick<_Block_, 'number'> }>;
};
