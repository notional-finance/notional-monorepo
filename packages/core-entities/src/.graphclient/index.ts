...
const notionalV3Handler = new GraphqlHandler({
              name: "NotionalV3",
              config: {"endpoint":"https://gateway-arbitrum.network.thegraph.com/api/f9f58a6131e8807672eaa304ed6abef8/subgraphs/id/{context.subgraphId:4oVxkMtN4cFepbiYrSKz1u6HWnJym435k5DQRAFt2vHW}","retry":2},
              baseDir,
              cache,
              pubsub,
...
export type HistoricalTradingActivityQuery = { tradingActivity: Array<(
    ...
    & { transactionHash: Pick<Transaction, 'id'>, transfers: Array<(
      ...
      & { to: Pick<Account, 'id'>, from: Pick<Account, 'id'>, token: Pick<Token, 'id' | 'currencyId' | 'maturity'> }
    )> }
  )>, _meta?: Maybe<{ block: Pick<_Block_, 'number'> }> };
...
export const AllOraclesDocument = gql`
  ...
  oracles(
    where: {oracleType_in: [Chainlink, fCashOracleRate, fCashSettlementRate, PrimeCashToUnderlyingExchangeRate, PrimeDebtToUnderlyingExchangeRate, VaultShareOracleRate, nTokenToUnderlyingExchangeRate, fCashSpotRate, nTokenFeeRate, VaultShareInterestAccrued, nTokenInterestAccrued], matured: false}
    first: 1000
    skip: $skip
  ) {
    id
