...
const walletUpdates$ = _walletUpdates.asObservable().pipe(
  distinctUntilChanged((a, b) => {
    return (
      ...
      a.size === b.size &&
      [...a.entries()].every(
        ([key, t1]) => b.has(key) && compareTokenBalances(t1, b.get(key)!)
      )
    );
  })
);
...
function compareTokenBalances(a: TokenBalance, b: TokenBalance) {
  ...
  const noAllowance = TypedBigNumber.from(
    0,
...
const _refreshWalletBalances$ = combineLatest([
  tokens$,
  system$,
  account$,
]).pipe(
...
async function getBalanceAndAllowance(
  ...
  if (contract) {
    ...
    const balance = TypedBigNumber.fromBalance(balanceBN, symbol, false);
    const allowance = TypedBigNumber.fromBalance(
