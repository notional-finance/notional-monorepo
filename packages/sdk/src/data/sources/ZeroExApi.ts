...
const fetchTradingEstimate = async (
  ...
  const estimates = (
    await Promise.all(
      sellRanges.map(async (a) => {
        try {
          const sellAmountString = parseUnits(
            a.toString(),
            sellToken.decimals
          ).toString();
          ...
          return {
            price: BigNumber.from(
              Math.floor(Number(v.price) * INTERNAL_TOKEN_PRECISION)
            ),
            estimatedPriceImpact: BigNumber.from(
              Math.floor(Number(v.price) * INTERNAL_TOKEN_PRECISION)
            ),
  ...
  return {
    ...
    buyTokenAddress: addressOverride
      ? addressOverride.buyToken.toLowerCase()
      : buyToken.address.toLowerCase(),
    sellTokenAddress: addressOverride
      ? addressOverride.sellToken.toLowerCase()
      : sellToken.address.toLowerCase(),
    estimates,
  };
};

export function getTradingEstimates(
  network: NETWORKS,
  skipFetchSetup: boolean
) {
  const _fetch = skipFetchSetup ? fetch : crossFetch;
  return Promise.all(
    RequiredEstimates[network].map(
      ({ buyToken, sellToken, sellRanges, addressOverride }) =>
        fetchTradingEstimate(
          buyToken,
          sellToken,
          sellRanges,
          network as NETWORKS,
...
export async function getTrade(
  ...
  return {
    ...
    guaranteedPrice: Math.floor(
      Number(v.guaranteedPrice) * INTERNAL_TOKEN_PRECISION
    ),
