...
function mapSecondaryBorrowCurrencies(
...
function mapSecondarySymbols(
  ...
  if (s.length === 2)
    return [
      s[0].underlyingSymbol || s[0].symbol,
      s[1].underlyingSymbol || s[1].symbol,
    ];
  return undefined;
}
...
export async function getSystemConfig(
  ...
  const data = await graphClient.queryOrThrow<SystemQueryResult>(
      ...
      (c) =>
        ({
          ...
          underlyingDecimals: c.underlyingDecimals
            ? BigNumber.from(c.underlyingDecimals)
            : undefined,
          underlyingDecimalPlaces: Math.log10(
            Number(c.underlyingDecimals ?? 1)
          ),
          ...
          nToken: c.nToken?.symbol
            ? {
                ...
                incentiveEmissionRate: BigNumber.from(
                  c.nToken?.incentiveEmissionRate || 0
                ),
              }
            : undefined,
          incentiveMigration: c.incentiveMigration
            ? {
                migratedEmissionRate: BigNumber.from(
                  c.incentiveMigration.migrationEmissionRate
                ),
                integralTotalSupply: BigNumber.from(
                  c.incentiveMigration.finalIntegralTotalSupply
                ),
                migrationTime: BigNumber.from(
                  c.incentiveMigration.migrationTime
                ).toNumber(),
              }
            : undefined,
          ethExchangeRate: {
          ...
          leveragedVaults: c.leveragedVaults.map((v) => {
            ...
            const secondarySymbols = mapSecondarySymbols(
              v.secondaryBorrowCurrencies
            );
            ...
            return {
              ...
              secondaryBorrowCurrencies: mapSecondaryBorrowCurrencies(
                v.secondaryBorrowCurrencies
              ),
              ...
              vaultStates: v.maturities.map((m) => {
                ...
                return {
                  ...
                  settlementRate: m.settlementRate?.rate
                    ? BigNumber.from(m.settlementRate.rate)
                    : undefined,
