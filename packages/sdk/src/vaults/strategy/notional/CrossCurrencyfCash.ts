...
const crossCurrencyInterface = new utils.Interface([
  'function LEND_CURRENCY_ID() view returns (uint16)',
]);
...
export default class CrossCurrencyfCash extends BaseVault<
  DepositParams,
  RedeemParams,
  InitParams
> {
  ...
  public getStrategyTokensFromValue(
    ...
    const lendCurrencyPV = valuation
      .toETH(false)
      .fromETH(this.lendCurrencyId, false);
    ...
    const lendCurrencyfCash = Market.fCashFromExchangeRate(
      Market.interestToExchangeRate(
        riskAdjustedOracleRate,
        blockTime,
        maturity
      ),
  ...
  public getLiquidationThresholds(
    ...
    const maxInterestRate = this.getVaultMarket(
      vaultAccount.maturity
    ).maxInterestRate();
  ...
  public async getDepositParametersExact(
    ...
    const { buyAmount, minPurchaseAmount, data } =
      await TradeHandler.getExactTrade(
  ...
  public getSlippageForDeposit(
    ...
    const idealTrade = TradeHandler.getIdealOutGivenIn(
      this.lendCurrencyId,
    ...
    return {
      likelySlippage: idealfCash
        .sub(likelyfCash)
        .scale(RATE_PRECISION, idealfCash)
        .toNumber(),
      worstCaseSlippage: idealfCash
        .sub(worstCasefCash)
        .scale(RATE_PRECISION, idealfCash)
        .toNumber(),
    };
  ...
  public getSlippageForRedeem(
    ...
    const idealCash = Market.cashFromExchangeRate(
      market.marketExchangeRate(blockTime),
      lendfCash
    );
    const idealTrade = TradeHandler.getIdealOutGivenIn(
      vault.primaryBorrowCurrency,
      idealCash.toExternalPrecision()
    );
    ...
    return {
      likelySlippage: idealTrade
        .sub(redeemAmount.toExternalPrecision())
        .scale(RATE_PRECISION, idealTrade)
        .toNumber(),
      worstCaseSlippage: idealTrade
        .sub(minPurchaseAmount)
        .scale(RATE_PRECISION, idealTrade)
        .toNumber(),
    };
  }
  ...
  public getStrategyTokensGivenRedeem(
    ...
    const lendfCash = market.getfCashAmountGivenCashAmount(
