...
class TypedBigNumber {
  ...
  get decimals() {
    ...
    if (this._decimalsOverride)
      return BigNumber.from(10).pow(this._decimalsOverride);
    if (this.currencyId === STAKED_NOTE_CURRENCY_ID)
      return ethers.constants.WeiPerEther;
    if (this.currencyId === NOTE_CURRENCY_ID)
      return BigNumber.from(INTERNAL_TOKEN_PRECISION);
    ...
    if (!decimals)
      throw new Error(`Decimals not found for currency ${this.currencyId}`);
    return decimals;
  }

  get hashKey() {
    return utils.id(
      [this.type, this.n.toString(), this._isWETH, this.symbol].join(':')
    );
  }
  ...
  static from(
    ...
    if (value._isBigNumber)
      return new TypedBigNumber(value, type, symbol, decimalsOverride);
    return new TypedBigNumber(
  ...
  checkType(type: BigNumberType) {
    if (this.type !== type)
      throw TypeError(`Invalid TypedBigNumber type ${this.type} != ${type}`);
  }
  ...
  check(type: BigNumberType, symbol: string | undefined) {
    ...
    if (this.symbol !== symbol)
      throw TypeError(
        `Invalid TypedBigNumber currency ${this.symbol} != ${symbol}`
      );
  }
  ...
  checkMatch(other: TypedBigNumber) {
    if (other.type !== this.type)
      throw TypeError(
        `Unmatched BigNumber types ${this.type} != ${other.type}`
      );
    if (this.symbol && this.symbol !== other.symbol) {
      throw TypeError(
        `Unmatched currency types ${this.symbol} != ${other.symbol}`
      );
    }
  }
  ...
  scale(
    numerator: BigNumberish | TypedBigNumber,
    divisor: BigNumberish | TypedBigNumber
  ): TypedBigNumber {
    ...
    if (
      numerator instanceof TypedBigNumber &&
      divisor instanceof TypedBigNumber
    ) {
      try {
  ...
  isAssetCash(): boolean {
    return (
      ...
      this.type === BigNumberType.ExternalAsset
    );
  }
  ...
  isUnderlying(): boolean {
    return (
      ...
      this.type === BigNumberType.ExternalUnderlying
    );
  }
  ...
  toExactString(): string {
    ...
    if (this.isStakedNOTE()) {
      ...
      decimalPlaces = this.isUnderlying()
  ...
  toAssetCash(
    internalPrecision: boolean = this.isInternalPrecision(),
    overrideRate?: BigNumber
  ): TypedBigNumber {
    if (this.isAssetCash()) {
      return internalPrecision
        ? this.toInternalPrecision()
        : this.toExternalPrecision();
    }
    if (this.isNonMintable()) {
      // A non mintable token does not require currency conversion
      const matchingPrecision = internalPrecision
        ? this.toInternalPrecision()
        : this.toExternalPrecision();
      const bnType = internalPrecision
        ? BigNumberType.InternalAsset
        : BigNumberType.ExternalAsset;
      return new TypedBigNumber(matchingPrecision.n, bnType, this.symbol);
    }
    if (this.isUnderlying()) {
      const { underlyingDecimalPlaces, assetRate: fetchedRate } =
        System.getSystem().getAssetRate(this.currencyId);
      const assetRate = overrideRate || fetchedRate;
      if (!underlyingDecimalPlaces || !assetRate)
        throw Error(`Asset rate for ${this.currencyId} not found`);

      const underlyingDecimals = BigNumber.from(10).pow(
    ...
    if (this.isNToken()) {
      ...
      const assetValue = NTokenValue.convertNTokenToInternalAsset(
        this.currencyId,
        this,
        false
      );
  ...
  toUnderlying(
    ...
    if (this.isNToken()) {
      ...
      return NTokenValue.convertNTokenToInternalAsset(
        this.currencyId,
        this,
        false
      ).toUnderlying(internalPrecision);
    }
  ...
  toETH(useHaircut: boolean) {
    const { haircut, buffer, latestRate, rateDecimalPlaces } =
      System.getSystem().getETHRate(this.currencyId);
    ...
    const bnType = this.isInternalPrecision()
      ? BigNumberType.InternalUnderlying
      : BigNumberType.ExternalUnderlying;
  ...
  fromETH(currencyId: number, useHaircut = false) {
    ...
    const { haircut, buffer, rateDecimalPlaces, latestRate } =
      System.getSystem().getETHRate(currencyId);
    // eslint-disable-next-line
    const underlyingSymbol =
      currencyId === NOTE_CURRENCY_ID
        ? 'NOTE'
        : System.getSystem().getUnderlyingSymbol(currencyId);
  ...
  toCUR(symbol: string) {
    ...
    return new TypedBigNumber(
      usdValue.scale(usdRate, ethers.constants.WeiPerEther).n,
      BigNumberType.Currency,
      symbol
    );
  }
