...
export default class BalanceSummary {
  ...
  public get hashKey() {
    return utils.id(
      `${
        this.currencyId
      }:${this.assetCashBalance.toString()}:${this.nTokenBalance?.toString()}`
    );
  }
  ...
  public get totalCTokenInterest() {
    const netUnderlyingDeposit = this.history.reduce(
      (s, b) =>
        s.add(
          b.assetCashValueUnderlyingAfter.sub(b.assetCashValueUnderlyingBefore)
        ),
  ...
  public get totalInterestAccrued() {
    ...
    if (this.nTokenValueUnderlying) {
      totalUnderlyingValue = this.assetCashValueUnderlying.add(
        this.nTokenValueUnderlying
      );
    } else {
  ...
  private static formatYieldRate(
    rate: number,
    locale = 'en-US',
    precision = 3
  ) {
  ...
  public getWithdrawAmounts(
    ...
    if (withdrawAmountInternalAsset.gt(this.maxWithdrawValueAssetCash)) {
      ...
      !canWithdrawNToken ||
      !this.nTokenBalance ||
      this.nTokenBalance.isZero()
    ) {
      // If the nToken cannot be withdrawn, only return cash amounts
      const cashWithdraw = TypedBigNumber.min(
      ...
      const requiredNTokenRedeem = withdrawAmountInternalAsset.sub(
        this.assetCashBalance
      );
      const nTokenRedeem = NTokenValue.getNTokenRedeemFromAsset(
        this.currencyId,
        requiredNTokenRedeem
      );
      ...
      const nTokenRedeemValue = NTokenValue.getAssetFromRedeemNToken(
        this.currencyId,
        this.nTokenBalance
      );
  ...
  public static build(accountData: AccountData, blockTime = getNowSeconds()) {
    ...
    return (
      accountData.accountBalances
        ...
        .filter(
          (b) =>
            !b.assetCashBalance.isZero() ||
            (b.nTokenBalance && b.nTokenBalance.isPositive())
        )
  ...
  private static getMaxWithdrawData(
    ...
    const totalAccountAssetValue = nTokenAssetPV
      ? balance.cashBalance.add(nTokenAssetPV)
      : balance.cashBalance;
    ...
    return fcLocalAsset.lt(totalAccountAssetValue)
      ? fcLocalAsset
      : totalAccountAssetValue;
  }
}
