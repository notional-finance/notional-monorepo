...
export default class NOTESummary {
  public get hashKey() {
    return utils.id(
      `${this.NOTEBalance.toString()}:${this.sNOTEBalance.toString()}`
    );
  }
  ...
  public static async build(account: Account) {
    ...
    const noteBalance = await System.getSystem()
      .getNOTE()
      .balanceOf(account.address);
    const sNoteBalance = await System.getSystem()
      .getStakedNote()
      .balanceOf(account.address);
  ...
  public getStakedNoteValue(): TypedBigNumber {
    if (this.sNOTEBalance.isZero())
      return TypedBigNumber.fromBalance(0, 'NOTE', false);
  ...
  public static getTransactionHistory(
    stakedNoteHistory?: StakedNoteHistory
  ): TransactionHistory[] {
  ...
  private getStakedNoteReturns() {
    ...
    const amountJoinedInNote = ethAmountJoined
      .fromETH(NOTE_CURRENCY_ID, false)
      .add(noteAmountJoined);
    const amountRedeemedInNote = ethAmountRedeemed
      .fromETH(NOTE_CURRENCY_ID, false)
      .add(noteAmountRedeemed);
    ...
    if (netCostBasis.isPositive()) {
      ...
      realizedYield =
        ((currentStakedNoteValue.n.div(netCostBasis.n).toNumber() -
          INTERNAL_TOKEN_PRECISION) /
          INTERNAL_TOKEN_PRECISION) *
        100;
    }
  ...
  private getUnclaimedNOTE() {
    ...
    const currentUnclaimedNOTE = this.accountData.accountBalances.reduce(
      (note, balance) => {
    ...
    const unclaimedNOTEIn5Min = this.accountData.accountBalances.reduce(
      (note, balance) => {
        if (balance.nTokenBalance?.isPositive()) {
          return note.add(
            NTokenValue.getClaimableIncentives(
    ...
    return {
      ...
      rateOfChangePerSecond:
        unclaimedNOTEIn5Min.sub(currentUnclaimedNOTE).toNumber() / 300,
    };
  }
}
