...
export default class AssetSummary {
  ...
  public get hashKey() {
    return utils.id(
      `${
        this.assetKey
      }:${this.fCash?.notional.toString()}:${this.liquidityToken?.notional.toString()}`
    );
  }
  ...
  constructor(
    ...
    if (!fCash && !liquidityToken)
      throw new Error('Invalid asset summary input');
    this.maturity = (fCash?.maturity || liquidityToken?.maturity) as number;
    this.currencyId = (fCash?.currencyId ||
      liquidityToken?.currencyId) as number;
    ...
    return rollMarkets.map((m) => {
      ...
      try {
        const { netCashToAccount } =
          matchingMarket.getCashAmountGivenfCashAmount(rollfCashAmount.neg());

        // Lend: Cash lent to new market, newfCash is positive
        // Borrow: Cash to borrow from new market, newfCash is negative
        const newfCash = m.getfCashAmountGivenCashAmount(
  ...
  public static build(accountData: AccountData) {
    ...
    return Object.keys(assetsReduced)
      .map((assetKey) => {
        ...
        const currency = system.getCurrencyById(
          liquidityToken?.currencyId || (fCash?.currencyId as number)
        );
  ...
  private static getAssetValue(
    ...
    if (!currencyId)
      throw new Error('Currency id not found when mapping assets');
    const cashGroup = system.getCashGroup(currencyId);

    // Get the underlying PV and fCash value of the asset
    let underlyingInternalPV = TypedBigNumber.from(
      0,
      BigNumberType.InternalUnderlying,
      underlyingSymbol
    );
    let fCashValue = TypedBigNumber.from(
      0,
      BigNumberType.InternalUnderlying,
      underlyingSymbol
    );
