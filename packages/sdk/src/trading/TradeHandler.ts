...
export default class TradeHandler {
  private static INTERNAL_TOKEN_SQUARED = BigNumber.from(
    INTERNAL_TOKEN_PRECISION
  ).mul(INTERNAL_TOKEN_PRECISION);

  private static _checkAmount(amount: TypedBigNumber) {
    if (!amount.isExternalPrecision())
      throw Error('Trading amounts must be external precision');
    if (!amount.isUnderlying())
      throw Error('Trading amounts must be underlying');
  }

  public static getIdealOutGivenIn(
    outCurrencyId: number,
    amountIn: TypedBigNumber
  ) {
    this._checkAmount(amountIn);
    if (outCurrencyId === amountIn.currencyId)
      throw Error('Matching currencies');
    return amountIn
      .toInternalPrecision()
      .toETH(false)
      .fromETH(outCurrencyId, false)
      .toExternalPrecision();
  }

  public static getIdealInGivenOut(
    inCurrencyId: number,
    amountOut: TypedBigNumber
  ) {
    this._checkAmount(amountOut);
    if (inCurrencyId === amountOut.currencyId)
      throw Error('Matching currencies');
    return amountOut
      .toInternalPrecision()
      .toETH(false)
      .fromETH(inCurrencyId, false)
      .toExternalPrecision();
  }

  public static getSellEstimate(
    sellToken: string,
    requiredBuyAmount: TypedBigNumber,
    slippageBPS = 0
  ) {
    const buyToken = requiredBuyAmount.symbol;
    if (sellToken === buyToken) throw Error('Matching currencies');
    const tradingEstimates = System.getSystem().getTradingEstimates(
      sellToken,
      buyToken
    );
    const expectedPrice = this._getExpectedPrice(
      tradingEstimates,
      requiredBuyAmount,
      'buy'
    );
    ...
    const sellEstimate = TypedBigNumber.fromBalance(
      requiredBuyAmount
        .scale(INTERNAL_TOKEN_PRECISION, expectedPrice)
        .toInternalPrecision().n,
      sellToken,
      true
    ).toExternalPrecision();
  ...
  public static getBuyEstimate(
    buyToken: string,
    requiredSellAmount: TypedBigNumber,
    slippageBPS = 0
  ) {
    ...
    const buyEstimate = TypedBigNumber.fromBalance(
      requiredSellAmount
        .scale(expectedPrice, INTERNAL_TOKEN_PRECISION)
        .toInternalPrecision().n,
      buyToken,
      true
    ).toExternalPrecision();
  ...
  private static _getExpectedPrice(
    ...
    const upperEstimateIndex = tradingEstimates.estimates.findIndex(
    ...
    if (upperEstimateIndex === -1) {
      ...
      const upperAmount =
        buyOrSell === 'sell'
          ? upperEstimate.sellAmount
          : upperEstimate.buyAmount;
      const lowerAmount =
        buyOrSell === 'sell'
          ? lowerEstimate.sellAmount
          : lowerEstimate.buyAmount;
      ...
      const slope = upperEstimate.price
        ...
        .div(
          upperAmount
            .toInternalPrecision()
            .sub(lowerAmount.toInternalPrecision()).n
        );
      const offset = requiredAmount
        .toInternalPrecision()
        .sub(lowerAmount.toInternalPrecision()).n;

      return slope
        .mul(offset)
        .div(this.INTERNAL_TOKEN_SQUARED)
        .add(lowerEstimate.price);
    }
  }

  public static async getExactTrade(
    sellToken: string,
    buyToken: string,
    requiredSellAmount: TypedBigNumber
  ) {
    if (sellToken === buyToken) throw Error('Matching currencies');
    const { sellTokenAddress, buyTokenAddress, estimates } =
      System.getSystem().getTradingEstimates(sellToken, buyToken);
    ...
    return {
      ...
      minPurchaseAmount: trade.sellAmount
        .mul(trade.guaranteedPrice)
