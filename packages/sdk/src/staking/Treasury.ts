...
export default class Treasury {
  private static async populateTxnAndGas(
    msgSender: string,
    methodName: string,
    methodArgs: any[]
  ) {
  ...
  public static async getReserveData() {
    const system = System.getSystem();
    const treasuryManager = system.getTreasuryManager();
    const results = await system.graphClient.queryOrThrow<ReserveQueryResult>(
      reserveQuery
    );
    const reserveResults = await Promise.all(
      results.cashGroups.map(async (r) => {
        ...
        const b = await (
          currency.underlyingContract || currency.assetContract
        ).balanceOf(treasuryManager.address);
        const treasuryBalance = TypedBigNumber.fromBalance(
          b,
          underlyingSymbol,
          false
        );
  ...
  public static async getCompData() {
    ...
    const compBalance =
      (await COMP?.balanceOf(treasuryManager.address)) || BigNumber.from(0);
    const results = await system.graphClient.queryOrThrow<CompQueryResult>(
      compReserveQuery
    );
  ...
  public static async harvestAssetsFromNotional(currencyIds: number[]) {
    ...
    return Treasury.populateTxnAndGas(manager, 'harvestAssetsFromNotional', [
      currencyIds,
    ]);
  }
  ...
  public static async harvestCOMPFromNotional(currencyIds: number[]) {
    const system = System.getSystem();
    const manager = await Treasury.getManager();
    const cTokens = currencyIds.map(
      (c) => system.getCurrencyById(c).assetContract.address
    );
    return Treasury.populateTxnAndGas(manager, 'harvestCOMPFromNotional', [
      cTokens,
    ]);
  }
  ...
  public static async investIntoStakedNOTE(
    noteAmount: TypedBigNumber,
    wethAmount: TypedBigNumber
  ) {
    ...
    const minBPT = StakedNote.getExpectedBPT(noteAmount, wethAmount)
      ...
      .div(RATE_PRECISION);
    return Treasury.populateTxnAndGas(manager, 'investWETHAndNOTE', [
      wethAmount.n,
      noteAmount.n,
      minBPT,
    ]);
  ...
  private static getMakerTokenAddress(symbol: string) {
    const address =
      symbol === 'COMP'
        ...
        : System.getSystem().getCurrencyBySymbol(symbol).underlyingContract
            ?.address;
  ...
  public static async getTradePriceData(symbol: string) {
    ...
    const treasuryManager = system.getTreasuryManager();
    const priceOracleAddress = await treasuryManager.priceOracles(
      makerTokenAddress
    );
    const slippageLimit = await treasuryManager.slippageLimits(
      makerTokenAddress
    );
    const priceOracle = new Contract(
      priceOracleAddress,
      IAggregatorABI,
      system.batchProvider
    ) as IAggregator;
