...
export default class StakedNote extends BalancerPool {
  ...
  public static async mintFromETH(
    ...
    return StakedNote.populateTxnAndGas(address, 'mintFromETH', [
      noteAmount.n,
      minBPT,
      ethOverrides,
    ]);
  ...
  public static async mintFromWETH(
    ...
    const minBPT = StakedNote.getExpectedBPT(noteAmount, ethAmount)
      .mul((1 - bptSlippagePercent) * RATE_PRECISION)
      .div(RATE_PRECISION);
    return StakedNote.populateTxnAndGas(address, 'mintFromWETH', [
      noteAmount.n,
      ethAmount.n,
      minBPT,
      overrides,
  ...
  public static async redeem(
    ...
    const minETH = ethClaim.scale(
      (1 - slippagePercent) * RATE_PRECISION,
      RATE_PRECISION
    );
    const minNOTE = noteClaim.scale(
      (1 - slippagePercent) * RATE_PRECISION,
      RATE_PRECISION
    );
  ...
  public static getPoolTokenShare(sNOTEAmount: TypedBigNumber) {
    ...
    const { sNOTETotalSupply, sNOTEBptBalance } =
   ...
   */
  public static async startCoolDown(
    address: string,
    overrides = {} as Overrides
  ) {
   ...
   */
  public static async stopCoolDown(
    address: string,
    overrides = {} as Overrides
  ) {
   ...
   */
  public static async accountCoolDown(
    address: string,
    _blockTime = getNowSeconds()
  ) {
    const { redeemWindowSeconds } =
      System.getSystem().getStakedNoteParameters();
    const sNOTE = System.getSystem().getStakedNote();
    ...
    const isInRedeemWindow =
      redeemWindowBegin.lte(blockTime) && blockTime.lte(redeemWindowEnd);
   ...
   */
  public static async canAccountRedeem(
    address: string,
    blockTime = getNowSeconds()
  ) {
    const { redeemWindowBegin, redeemWindowEnd } =
      await StakedNote.accountCoolDown(address, blockTime);
    return (
      !redeemWindowBegin.isZero() &&
      redeemWindowBegin.lte(blockTime) &&
      redeemWindowEnd.gte(blockTime)
    );
  }
}
