...
export default class BalancerPool {
  ...
  public static getExpectedBPT(
    ...
    const ethBalanceRatioWithFee = ethBalance
      .add(ethAmount)
      .scale(BalancerPool.BPT_PRECISION, ethBalance.n).n;
    ...
    const invariantRatioWithFees = ethBalanceRatioWithFee
      ...
      .add(
        noteBalanceRatioWithFee
          .mul(BalancerPool.NOTE_WEIGHT)
          .div(BalancerPool.BPT_PRECISION)
      );
    ...
    if (ethBalanceRatioWithFee.gt(invariantRatioWithFees)) {
      ...
      ethInWithoutFee = nonTaxableAmount.add(
        taxableAmount.scale(
          BalancerPool.BPT_PRECISION.sub(swapFee),
          BalancerPool.BPT_PRECISION
        )
      );
    } else {
    ...
    const balanceRatioEth = ethBalance
      .add(ethInWithoutFee)
      .scale(BalancerPool.BPT_PRECISION, ethBalance.n);
    let invariantRatio = ethers.utils.parseUnits(
      (parseFloat(balanceRatioEth.toExactString()) ** 0.2).toFixed(18),
      18
    );
    ...
    const balanceRatioNote = noteBalance
      .add(noteInWithoutFee)
      .scale(INTERNAL_TOKEN_PRECISION, noteBalance.n);
  ...
  public static spotPriceToUSD(spotPrice: BigNumber) {
    return TypedBigNumber.fromBalance(spotPrice, 'ETH', false)
      .toInternalPrecision()
      .toUSD();
  }

  public static getExpectedPriceImpact(
    noteAmount: TypedBigNumber,
    ethAmount: TypedBigNumber
  ) {
    const { ethBalance, noteBalance } =
    ...
    const ethRatio = ethBalance
      .add(ethAmount)
      .scale(BalancerPool.BPT_PRECISION, BalancerPool.ETH_WEIGHT).n;
  ...
  public static getStakedNOTEExchangeRate() {
    ...
    return ethValue
      .add(noteValue.toETH(false))
      .scale(this.BPT_PRECISION, sNOTETotalSupply);
  }
  ...
  public static getStakedNOTEPoolValue() {
    ...
    return {
      ethValue,
      noteValue,
      usdValue: ethValue.toUSD().add(noteValue.toUSD()),
    };
  }

  public static getBptValue() {
    const { ethBalance, noteBalance, balancerPoolTotalSupply } =
      System.getSystem().getStakedNoteParameters();
