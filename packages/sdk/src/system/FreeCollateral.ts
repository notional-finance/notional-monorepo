...
export default class FreeCollateral {
   ...
   */
  public static getFreeCollateral(
    account: AccountData,
    ...
    account.accountBalances.forEach((b) => {
      ...
      if (value.isNegative()) {
        ...
        netETHDebtWithBuffer = netETHDebtWithBuffer.add(
          value.toETH(useHaircut).abs()
        );
      } else {
        netETHCollateralWithHaircut = netETHCollateralWithHaircut.add(
          value.toETH(useHaircut)
        );
  ...
  public static getCurrencyComponents(
    ...
    let nTokenValue = TypedBigNumber.from(
      0,
  ...
  public static getNetfCashPositions(
    ...
    const fCashAssets = portfolio
      .filter(
        (a) => a.currencyId === currencyId && a.assetType === AssetType.fCash
      )
      .map((a) => ({ ...a }));
    ...
    portfolio
      ...
      .forEach((lt) => {
        ...
        const index = fCashAssets.findIndex(
          (a) => a.assetType === AssetType.fCash && lt.maturity === a.maturity
        );
        if (index > -1) {
          // net off fCash if it exists
          fCashAssets[index].notional =
            fCashAssets[index].notional.add(fCashClaim);
        } else {
          fCashAssets.push({
            currencyId,
  ...
  public static getBorrowCapacity(
    ...
    const {
      ...
      netETHDebtWithBuffer,
      netUnderlyingAvailable,
    } = this.getFreeCollateral(accountDataCopy, blockTime);
    const { buffer } = System.getSystem().getETHRate(borrowCurrencyId);
    const netFreeCollateral =
      netETHCollateralWithHaircut.sub(netETHDebtWithBuffer);
    const netLocal =
      netUnderlyingAvailable.get(borrowCurrencyId) ||
    ...
    if (netLocal.isPositive()) {
      if (netLocal.toETH(true).gt(netFreeCollateral)) {
        ...
        additionalBorrowCapacity = netFreeCollateral.fromETH(
          borrowCurrencyId,
          true
        );
      } else {
      ...
      additionalBorrowCapacity = netFreeCollateral
        .fromETH(borrowCurrencyId, false)
        .scale(100, buffer);
    }
    ...
    let usedBorrowCapacity = accountData.portfolio
      .filter(
        (a) => a.currencyId === borrowCurrencyId && a.notional.isNegative()
      )
      .reduce(
        (s, a) => s.add(a.notional.abs()),
        TypedBigNumber.getZeroUnderlying(borrowCurrencyId)
      );

    if (cashBalance?.isNegative()) {
      usedBorrowCapacity = usedBorrowCapacity.add(
        cashBalance.toUnderlying(true).abs()
      );
    }

    const totalBorrowCapacity =
      additionalBorrowCapacity.add(usedBorrowCapacity);
    return {
      totalBorrowCapacity,
      usedBorrowCapacity,
  ...
  public static calculateBorrowRequirement(
    ...
    if (bufferedRatio < 100)
      throw new RangeError('Buffered ratio must be more than 100');
    ...
    let { minCollateral, targetCollateral } =
      FreeCollateral.calculateTargetCollateral(
        netETHCollateralWithHaircut,
        netETHDebtWithBuffer,
    ...
    if (mintNTokenCollateral) {
      ...
      const minAssetCash = minCollateral
        .toAssetCash(useInternal)
        .scale(100, nToken.pvHaircutPercentage);
      const targetAssetCash = targetCollateral
        .toAssetCash(useInternal)
        .scale(100, nToken.pvHaircutPercentage);
      minCollateral = NTokenValue.getNTokensToMint(
        collateralCurrencyId,
        minAssetCash
      );
      targetCollateral = NTokenValue.getNTokensToMint(
        collateralCurrencyId,
        targetAssetCash
      );
      ...
      minCollateralCopy.updateBalance(
        collateralCurrencyId,
        TypedBigNumber.getZeroUnderlying(collateralCurrencyId).toAssetCash(
          useInternal
        ),
        minCollateral
      );
      ...
      minCollateralCopy.updateBalance(
        collateralCurrencyId,
        minCollateral.toAssetCash(useInternal)
      );
      targetCollateralCopy.updateBalance(
        collateralCurrencyId,
        targetCollateral.toAssetCash(useInternal)
      );
    }

    const minFC = FreeCollateral.getFreeCollateral(
      minCollateralCopy,
      blockTime
    );
    const targetFC = FreeCollateral.getFreeCollateral(
  ...
  public static calculateTargetCollateral(
    ...
    let targetEthRequired = netETHDebtWithBuffer
      .scale(bufferedRatio, 100)
      .sub(netETHCollateralWithHaircut);
    ...
    if (collateralNetAvailable.n.gte(0)) {
      ...
      return {
        ...
        targetCollateral: targetEthRequired.fromETH(
          collateralCurrencyId,
          useHaircut
        ),
      };
    ...
    const collateralDebtETHBuffer = collateralNetAvailable
      .toETH(useHaircut)
      .abs();
  ...
  private static getRequiredCollateral(
    ...
    const system = System.getSystem();
    const { buffer: collateralBuffer } =
      system.getETHRate(collateralCurrencyId);
    ...
    const postDebt = netETHDebtWithBuffer
      .sub(collateralDebtETHBuffer)
      .scale(bufferedRatio, 100);
    const collateralHaircut = system.getETHRate(collateralCurrencyId)?.haircut;
    const additionalCollateralETH = postDebt
      .sub(netETHCollateralWithHaircut)
      .scale(100, collateralHaircut);
   ...
   */
  public static calculateCollateralRatio(
    netETHCollateral: TypedBigNumber,
    ...
    const collateralRatioNumber = netETHCollateral
      .scale(INTERNAL_TOKEN_PRECISION, netETHDebt.n)
      .toNumber();
