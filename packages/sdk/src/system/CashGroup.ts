...
export default class CashGroup {
  ...
  public get blockSupplyRate() {
    return System.getSystem()
      .getAnnualizedSupplyRate(this.currencyId)
      .toNumber();
  }
  ...
  constructor(currency: Currency, cashGroupData: CashGroupData) {
    ...
    this.rateOracleTimeWindowSeconds =
    ...
    this.liquidityTokenHaircutsPercent =
      cashGroupData.liquidityTokenHaircutsPercent.map((l) => l);
  ...
  public static getMarketIndexForMaturity(
    maturity: number,
    blockTime = getNowSeconds()
  ) {
    for (let i = 1; i <= 7; i += 1) {
      if (maturity === CashGroup.getMaturityForMarketIndex(i, blockTime))
        return i;
    }
  ...
  public static getSettlementDate(assetType: AssetType, maturity: number) {
    ...
    const marketLength = CashGroup.getMarketMaturityLengthSeconds(
      assetTypeNum(assetType) - 1
    );
    return maturity - marketLength + SECONDS_IN_QUARTER;
  }

  public static getMaturityForMarketIndex(
    marketIndex: number,
    blockTime = getNowSeconds()
  ) {
  ...
  public getOracleRate(
    ...
    if (markets[index].maturity === maturity)
      return markets[index].marketOracleRate(blockTime);
    ...
    return Math.trunc(
      ((longRate - shortRate) * (maturity - shortMaturity)) /
  ...
  public getRiskAdjustedOracleRate(
    ...
    let oracleRate = this.getOracleRate(
      maturity,
      blockTime,
      marketOverrides,
  ...
  public getfCashPresentValueUnderlyingInternal(
    ...
    const exponent =
      -Math.trunc((oracleRate * timeToMaturity) / SECONDS_IN_YEAR) /
      RATE_PRECISION;
  ...
  public getLiquidityTokenValue(
    ...
    if (useHaircut) {
      return {
        fCashClaim: fCashClaim.scale(
          this.liquidityTokenHaircutsPercent[index],
          PERCENTAGE_BASIS
        ),
        assetCashClaim: assetCashClaim.scale(
          this.liquidityTokenHaircutsPercent[index],
          PERCENTAGE_BASIS
        ),
      };
