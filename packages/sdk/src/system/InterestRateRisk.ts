...
export default class InterestRateRisk {
  public static calculateInterestRateRisk(
    ...
    return currencies.reduce(
      (map, id) => {
        const currentWeightedAvgInterestRate =
          InterestRateRisk.getWeightedAvgInterestRate(id, blockTime);
        const netUnderlying =
          netUnderlyingAvailable.get(id) ||
          TypedBigNumber.getZeroUnderlying(id);
        const minLocalCollateral =
          InterestRateRisk.getMinLocalCurrencyCollateral(
            id,
            aggregateFC,
            netUnderlying
          );

        const upperLiquidationInterestRate =
          InterestRateRisk.findLiquidationRate(
            id,
            accountData,
            minLocalCollateral,
            true,
        ...
        const lowerLiquidationInterestRate =
          InterestRateRisk.findLiquidationRate(
            id,
            accountData,
            minLocalCollateral,
        ...
        if (
          upperLiquidationInterestRate !== null ||
          lowerLiquidationInterestRate !== null
        ) {
          map.set(id, {
  ...
  public static getWeightedAvgInterestRate(
    ...
    const { numerator, totalLiquidity } = markets.reduce(
      ({ numerator, totalLiquidity }, m) => {
        const oracleRate = cashGroup.getOracleRate(
          m.maturity,
          blockTime,
          markets
        );
    ...
    return totalLiquidity.isZero()
      ? null
      : numerator.div(totalLiquidity).toNumber();
  }
  ...
  public static getRiskyCurrencies(accountData: AccountData) {
    ...
    accountData.accountBalances.forEach((b) => {
      if (b.nTokenBalance?.isPositive()) {
        ...
        b.cashBalance.isPositive() &&
        debtCurrencies.has(b.currencyId)
      ) {
  ...
  public static getMinLocalCurrencyCollateral(
    ...
    const necessaryLocalCurrencyGainOrLoss = aggregateFC.fromETH(
      currencyId,
      true
    );
  ...
  public static findLiquidationRate(
    ...
    const portfolio = accountData.portfolio.filter(
      (a) => a.currencyId === currencyId
    );
    const cashBalance =
      accountData.cashBalance(currencyId) ||
      TypedBigNumber.getZeroUnderlying(currencyId);
    const nTokenBalance = accountData.nTokenBalance(currencyId);
    const maxInterestRate = InterestRateRisk.getMaxInterestRate(
      currencyId,
      blockTime
    );

    let simulatedLocalCollateral: TypedBigNumber;
    const startingInterestRate = fromMaxRate
      ? maxInterestRate
      : MIN_INTEREST_RATE;
    ...
    while (true) {
      ...
      if (
        simulatedLocalCollateral.gt(minLocalCollateral) ||
        simulatedLocalCollateral.isZero()
      ) {
  ...
  public static simulateLocalCurrencyValue(
    ...
    if (nTokenBalance) {
      nTokenValue = InterestRateRisk.getNTokenSimulatedValue(
        nTokenBalance,
        marketOverrides
      );
    } else {
  ...
  public static getNTokenSimulatedValue(
    ...
    const { nToken, totalSupply } = NTokenValue.getNTokenFactors(
