...
describe('calculates free collateral', () => {
  ...
  describe('total borrow capacity', () => {
    it('calculates with no net local', () => {
      ...
      const { usedBorrowCapacity, totalBorrowCapacity } =
        FreeCollateral.getBorrowCapacity(3, accountData);
  ...
  describe('calculates free collateral', () => {
    ...
    it('handles nontradable assets', () => {
      ...
      expect(netETHCollateralWithHaircut.toNumber()).toBeLessThanOrEqual(
        0.78e8
      );
    ...
    it('for positive fcash', () => {
      ...
      expect(
        netETHCollateralWithHaircut.n
          .sub(BigNumber.from(0.95e8))
          .abs()
          .toNumber()
      ).toBeLessThanOrEqual(0.051e8);
      expect(
        netUnderlyingAvailable.get(2)!.scale(1, 100).toNumber()
      ).toBeCloseTo(netETHCollateralWithHaircut.scale(100, 92).toNumber(), -1);
    });
    ...
    it('for negative fcash', () => {
      ...
      expect(
        netETHDebt.n.sub(BigNumber.from(1e8)).abs().toNumber()
      ).toBeLessThanOrEqual(0.03e8);
      // Should be buffered by 109
      expect(
        Math.abs(netETHDebtWithBuffer.scale(100, netETHDebt.n).toNumber() - 109)
      ).toBeLessThanOrEqual(1);
      expect(netETHCollateralWithHaircut.isZero()).toBeTruthy();
      expect(netUnderlyingAvailable.get(2)!.scale(1, 100).toString()).toBe(
        netETHDebt.neg().toString()
      );
    });
    ...
    it('for liquidity token', () => {
      ...
      expect(netETHDebt.isZero()).toBeTruthy();
      expect(netETHDebtWithBuffer.isZero()).toBeTruthy();
      expect(netETHCollateralWithHaircut.toNumber()).toEqual(
        Math.trunc((29624839724 / 100) * 0.92)
      );
    ...
    it('for cash balances and ntokens with fcash', () => {
      ...
      expect(netETHDebt.isZero()).toBeTruthy();
      expect(netETHDebtWithBuffer.isZero()).toBeTruthy();
      expect(netETHCollateralWithHaircut.toNumber()).toBeGreaterThan(
        1.98e8 * 0.92
      );
    ...
    it('for negative cash balances with fcash', () => {
      ...
      expect(netETHCollateralWithHaircut.toNumber()).toBeLessThan(
        0.17e8 * 0.92
      );
  ...
  describe('borrow requirements', () => {
    ...
    beforeEach(() => {
      ...
      borrowfCashAmount = TypedBigNumber.from(
        -100e8,
        BigNumberType.InternalUnderlying,
        'DAI'
      );
    ...
    it('calculates borrowing requirements for stable / crypto with no account data', () => {
      ...
      const ethCollateral =
        (-borrowAmountHaircutPV * minCollateralRatio!) / (100 * 100);
      const ethTargetCollateral =
        (-borrowAmountHaircutPV * targetCollateralRatio!) / (100 * 100);
    ...
    it('calculates borrowing requirements for stable / crypto with nTokens', () => {
      ...
      const ethCollateral =
        (-borrowAmountHaircutPV * minCollateralRatio! * 50) / (100 * 100 * 0.9);
      const ethTargetCollateral =
        (-borrowAmountHaircutPV * targetCollateralRatio! * 50) /
        (100 * 100 * 0.9);
      expect(minCollateral.toNumber()).toBeCloseTo(ethCollateral, -3);
      expect(targetCollateral.toNumber()).toBeCloseTo(ethTargetCollateral, -3);
    });
    ...
    it('calculates borrowing requirements when local collateral can partially net off', () => {
      ...
      const usdcTargetCollateral =
        ((borrowAmountHaircutPV + 50e8) * -2.18) / 0.92;
      expect(minCollateral.toNumber()).toBeCloseTo(usdcCollateral, -3);
      expect(targetCollateral.toNumber()).toBeCloseTo(usdcTargetCollateral, -3);
    });
    ...
    it('calculates borrowing requirements when collateral is positive', () => {
      ...
      const usdcTargetCollateral =
        (borrowAmountHaircutPV * -2.18) / 0.92 - 10e8;
      expect(minCollateral.toNumber()).toBeCloseTo(usdcCollateral, -3);
      expect(targetCollateral.toNumber()).toBeCloseTo(usdcTargetCollateral, -3);
      expect(minCollateralRatio).toBeCloseTo(109 / 0.92, -1);
      expect(targetCollateralRatio).toBeCloseTo(minCollateralRatio! * 2, -1);
    ...
    it('calculates borrowing requirements when more collateral than ratio', () => {
      ...
      const collateralRatioBefore = FreeCollateral.calculateCollateralRatio(
        netETHCollateral,
        netETHDebt
      );
    ...
    it('calculates borrowing requirements when net collateral is negative but no eth required', () => {
      ...
      expect(
        FreeCollateral.calculateCollateralRatio(netETHAfter, netDebtAfter)
      ).toBeCloseTo(200, 0);
    });
    ...
    it('calculates borrowing requirements when collateral is negative', () => {
      ...
      // Expect that the buffered collateral ratio afterwards is about 200
      expect(
        FreeCollateral.calculateCollateralRatio(
          netETHCollateralWithHaircut,
          netETHDebtWithBuffer
        )
      ).toBeCloseTo(200, 0);
    ...
    it('it nets off fcash before calculating collateral requirement', () => {
      ...
      const { targetCollateral, minCollateral } =
        FreeCollateral.calculateBorrowRequirement(
          collateralCurrencyId,
          200,
          accountData,
    ...
    it('can override nToken asset cash PV', () => {
      ...
      system.setNTokenAssetCashPVProvider(collateralCurrencyId, {
        getNTokenAssetCashPV: () =>
          TypedBigNumber.from(5000000e8, BigNumberType.InternalAsset, 'cUSDC'),
      });
