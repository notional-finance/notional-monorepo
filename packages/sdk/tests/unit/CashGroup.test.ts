...
describe('Cash Group', () => {
  ...
  beforeAll(() => {
    ...
    for (let i = 1; i <= 7; i += 1) {
      ...
      markets.push({
        totalAssetCash: TypedBigNumber.fromBalance(
          ethers.constants.WeiPerEther.mul(2),
          'cETH',
          true
        ),
        totalLiquidity: TypedBigNumber.fromBalance(
          ethers.constants.WeiPerEther,
          'cETH',
          true
        ),
        totalfCash: TypedBigNumber.fromBalance(
          ethers.constants.WeiPerEther,
          'ETH',
          true
        ),
  ...
  it('gets the correct market index and maturity', () => {
    ...
    const newMaturity = CashGroup.getMaturityForMarketIndex(
      marketIndex,
      blockTime
    );
  ...
  it('gets the oracle rate for a specified market', () => {
    for (let i = 1; i <= 7; i += 1) {
      ...
      expect(cashGroup.getOracleRate(maturity, blockTime)).toBeCloseTo(
        oracleRate,
        -5
      );
    }
  ...
  it('linearly interpolates oracle rates between markets', () => {
    ...
    const maturity =
      Math.trunc((longMaturity - shortMaturity) / 2) + shortMaturity;
  ...
  it('gets discounted positive fcash values', () => {
    ...
    const fCash = TypedBigNumber.from(
      100e8,
      BigNumberType.InternalUnderlying,
      'ETH'
    );
    const discounted = cashGroup.getfCashPresentValueUnderlyingInternal(
      maturity,
      fCash,
      false,
      blockTime
    ...
    const annualizedRate = Market.exchangeToInterestRate(
      Market.exchangeRate(fCash, discounted),
      blockTime,
      maturity
    );
  ...
  it('gets haircut discounted positive fcash values', () => {
    ...
    const oracleRate =
      cashGroup.getOracleRate(maturity, blockTime) + fCashHaircutBasisPoints;
    const diff = Math.abs(annualizedRate - oracleRate);
  ...
  it('gets haircut discounted negative fcash values', () => {
    ...
    const oracleRate =
      cashGroup.getOracleRate(maturity, blockTime) - debtBufferBasisPoints;
    const diff = Math.abs(annualizedRate - oracleRate);
