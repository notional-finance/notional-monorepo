...
describe('Account Data', () => {
  ...
  describe('hashKey', () => {
    ...
    it('should return different hash keys for an account with different balances', () => {
      ...
      testAcct1.accountBalances[0].cashBalance = TypedBigNumber.from(
        5001e8,
        BigNumberType.InternalAsset,
        'cETH'
      );
  ...
  it('fails to update when balances dont match types', () => {
    ...
    expect(() =>
      accountDataCopy.updateBalance(
        1,
        TypedBigNumber.from(-100e8, BigNumberType.ExternalAsset, 'cDAI')
      )
    ).toThrowError();
  ...
  describe('loan to value ratio', () => {
    it('no debt', () => {
      const { totalETHDebts, totalETHValue, loanToValue } =
        accountData.loanToValueRatio();
    ...
    it('haircut ltv is 100 when fc is zero', () => {
      ...
      expect(
        netETHCollateralWithHaircut.sub(netETHDebtWithBuffer).isZero()
      ).toBeTruthy();
      const { haircutLoanToValue, maxLoanToValue, loanToValue } =
        accountData2.loanToValueRatio();
  ...
  describe('liquidation price', () => {
    it('gets liquidation price with ETH collateral', () => {
      ...
      } = account.getLiquidationPenalty(1, liquidationPrice!);

      expect(totalPenaltyRate).toBe(4);
      expect(totalPenalty?.symbol).toBe('ETH');
      const ethValue =
        account.cashBalance(1)!.toUnderlying().toFloat() * 0.4 * 0.04;
      expect(totalPenalty?.toFloat()).toBeCloseTo(ethValue);
      expect(totalPenaltyETHValueAtLiquidationPrice?.symbol).toBe('ETH');
      expect(totalPenaltyETHValueAtLiquidationPrice?.toFloat()).toBeCloseTo(
        totalPenalty!.toFloat()
      );
    });
    ...
    it('gets liquidation price with USDC collateral', () => {
      ...
      const {
        ...
        totalPenaltyRate,
        totalPenaltyETHValueAtLiquidationPrice,
      } = account.getLiquidationPenalty(3, liquidationPrice!);

      expect(totalPenaltyRate).toBe(4);
      expect(totalPenalty?.symbol).toBe('USDC');
      const usdcValue =
        account.cashBalance(3)!.toUnderlying().toFloat() * 0.4 * 0.04;
      ...
      expect(
        totalPenaltyETHValueAtLiquidationPrice?.toUSD().toFloat()
      ).toBeCloseTo(totalPenalty!.toFloat() / 2);
    });
    ...
    it('gets liquidation penalty with nUSDC collateral', () => {
      ...
      const liquidationPrice = account.getLiquidationPrice(3, 2);
      const { totalPenalty, totalPenaltyRate } = account.getLiquidationPenalty(
        3,
        liquidationPrice!
      );
