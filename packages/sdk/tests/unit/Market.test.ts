...
describe('Market', () => {
  ...
  it('calculates the max interest rate, 18%', () => {
    ...
    expect(market.maxInterestRate(blockTime) / RATE_PRECISION).toBeCloseTo(
      0.239,
      3
    );
  });
  ...
  it('converts between interest rates and exchange rates', () => {
    const annualRate = Market.exchangeToInterestRate(
      1.1e9,
      blockTime,
      maturity
    );
  ...
  it('converts between fcash and cash from exchange rates', () => {
    ...
    const cash = TypedBigNumber.from(
      100e8,
      BigNumberType.InternalUnderlying,
      'DAI'
    );
    ...
    expect(Market.fCashFromExchangeRate(exchangeRate, cash).toString()).toBe(
      fCash.toString()
    );
    expect(Market.cashFromExchangeRate(exchangeRate, fCash).toString()).toBe(
      cash.toString()
    );
  });
  ...
  it('calculates market oracle rates in the past', () => {
    ...
    expect(market.marketOracleRate(blockTime - 60 * 2000)).toEqual(
      lastImpliedRate
    );
  });
  ...
  it('gets market interest rates', () => {
    ...
    expect(
      Math.abs(
        exchangeRate -
          Market.interestToExchangeRate(interestRate, blockTime, maturity)
      )
    ).toBeLessThan(100);
  ...
  it('calculates fcash to cash for borrowing', () => {
    ...
    const fCashAmount = market.getfCashAmountGivenCashAmount(
      netCashToAccount,
      blockTime
    );
    expect(
