...
describe('Test Trade Handler', () => {
  ...
  it('gets trading estimates', () => {
    ...
    expect(
      System.getSystem().getTradingEstimates(
  ...
  it('returns a buy estimate below the minimum threshold', () => {
    ...
    expect(buyEstimate.toFloat() * 0.995).toBeCloseTo(
      minPurchaseAmount.toFloat()
    );
  });
  ...
  it('throws on a buy estimate above the maximum threshold', () => {
    expect(() => {
      TradeHandler.getBuyEstimate(
        'ETH',
        TypedBigNumber.fromBalance(10_000_100e8, 'DAI', true),
        0.005
      );
    }).toThrow();
  });
  ...
  it('returns a sell estimate below the minimum threshold', () => {
    ...
    expect(sellEstimate.toFloat() * 1.005).toBeCloseTo(
      minPurchaseAmount.toFloat()
    );
  });
  ...
  it('throws on a buy estimate above the maximum threshold', () => {
    expect(() => {
      TradeHandler.getBuyEstimate(
        'DAI',
        TypedBigNumber.fromBalance(6000e8, 'ETH', true),
        0.005
      );
    }).toThrow();
  });
