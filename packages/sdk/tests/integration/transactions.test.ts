...
describe('transactions', () => {
  ...
  it('executes a batch lend transaction', async () => {
    const contract =
      notional.system.getCurrencyBySymbol('DAI').underlyingContract;
    await contract
      ?.connect(daiWhale)
      .approve(
        notional.system.getNotionalProxy().address,
    ...
    const { portfolio } = await notional.system
      .getNotionalProxy()
      .connect(daiWhale)
      .getAccount(DAI_WHALE);
