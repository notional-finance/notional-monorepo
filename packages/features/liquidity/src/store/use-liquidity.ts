...
export function useLiquidity() {
  ...
  const blendedYield = currencyId
    ? NTokenValue.getNTokenBlendedYield(currencyId)
    : 0;
  const incentiveYield = currencyId
    ? NTokenValue.getNTokenIncentiveYield(currencyId)
    : 0;
  const totalYield = blendedYield + incentiveYield;
  let nTokensMinted = nTokenSymbol
    ? TypedBigNumber.fromBalance(0, nTokenSymbol, true)
    : undefined;
  let nTokenShare = 0;

  if (currencyId && nTokenSymbol && inputAmount) {
    nTokensMinted = NTokenValue.getNTokensToMint(
      currencyId,
    ...
    nTokenShare =
      convertRateToFloat(
        nTokensMinted?.scale(RATE_PRECISION, totalSupply.n).toNumber()
      ) || 0;
  }
  ...
  const canSubmit =
    !!inputAmount && inputAmount.isPositive() && hasError === false;
