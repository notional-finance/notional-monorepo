...
import { formatCryptoWithFiat, formatNumberAsPercent } from '@notional-finance/utils';
...
export const useBorrowCapacityTable = () => {
  ...
  const tableColumns: Record<string, any>[] = [
    ...
    {
      Header: (
        <FormattedMessage defaultMessage="Capacity Used" description={'Capacity Used header'} />
      ),
      Cell: MultiValueCell,
    ...
    {
      Header: (
        <FormattedMessage defaultMessage="Total Capacity" description={'Total Capacity header'} />
      ),
      Cell: MultiValueCell,
    ...
    {
      Header: (
        <FormattedMessage defaultMessage="Percent Used" description={'Percent Used header'} />
      ),
    ...
    {
      Header: (
        <FormattedMessage defaultMessage="Borrow Capacity" description={'Borrow Capacity header'} />
      ),
      Cell: SliderCell,
  ...
  const tableData = borrowCapacity?.map((data) => {
    const usedBorrowCapacityPercent =
      (data.usedBorrowCapacity.toFloat() / data.totalBorrowCapacity.toFloat()) * 100;
    return {
