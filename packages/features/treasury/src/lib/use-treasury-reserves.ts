...
export const useTreasuryReservesTable = () => {
  ...
  const tableData =
    reserveData?.map((r) => {
      return {
        ...
        reserveBalanceUSD: r.reserveBalance?.toInternalPrecision().toUSD().toDisplayString(),
  ...
  if (compReserveData) {
    ...
    tableData.push({
      ...
      reserveBalance: formatBigNumberToDecimals(compReserveData.reserveBalance, 18, 3),
      reserveBalanceCToken: '',
      reserveBuffer: formatBigNumberToDecimals(compReserveData.reserveBuffer, 18, 3),
      reserveBufferCToken: '',
      reserveBalanceUSD: formatBigNumberToDecimals(compReserveData.reserveValueUSD, 18, 3),
      treasuryBalance: formatBigNumberToDecimals(compReserveData.treasuryBalance, 18, 3),
    });
  }
