...
export const MainContent = () => {
  ...
  const { totalStakedNOTEValueUSD, annualInvestmentRate, stakedNOTEApy } =
    useStakedNoteReturns();
  return (
    <>
      <TradeSummaryContainer>
        <Box sx={{ margin: '0 15px' }}>
          <TradeActionHeader
            actionText={<FormattedMessage defaultMessage="Stake" />}
            token="sNOTE"
          />
          <TradeActionTitle
            title="APY"
        ...
        >
          <BoxDisplay
            title={
              <FormattedMessage {...messages.summary.totalStakedNoteValue} />
            }
      ...
      <MobileTradeActionSummary
        tradeAction={NOTIONAL_CATEGORIES.STAKE}
        selectedToken={'sNOTE'}
        dataPointOne={totalStakedNOTEValueUSD}
        dataPointOnePrefix="$"
        dataPointOneSuffix=" USD"
        dataPointTwo={annualInvestmentRate}
        dataPointTwoPrefix="$"
        dataPointTwoSuffix=" USD"
        fixedAPY={stakedNOTEApy}
      />
    </>
  );
};
