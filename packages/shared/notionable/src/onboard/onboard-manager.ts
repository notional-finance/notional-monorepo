...
import { distinctUntilChanged, share, catchError, mergeMap, map } from 'rxjs/operators';
import { chains as supportedChains, chainIds as supportedChainIds } from '../chains';
...
export async function connectWallet(label?: string) {
  try {
    const opts = label ? { autoSelect: { label, disableModals: true } } : undefined;
...
export async function resetAllWallets() {
  ...
  await Promise.all(wallets.map(({ label }) => onboard.disconnectWallet({ label })));
}
...
function hasWalletChanged([prev]: WalletState[], [curr]: WalletState[]) {
  if (prev && curr) {
    ...
    return prevAccount.address === currAccount.address && prevChain.id === currChain.id;
  }
  return false;
}
...
function getOnboardOptions({ enableAccountCenter = false, container = '#root' }: OnboardOptions) {
  return {
    accountCenter: {
      desktop: {
...
account$.subscribe((account) => {
  if (onboard && account) {
    ...
    if (wallets && wallets.length > 0) {
      ...
      const [acct] = accounts;
      if (acct && acct.address && acct.address.toLowerCase() !== account.address.toLowerCase()) {
        resetAllWallets();
      }
    }
  }
  ...
  .pipe(
    ...
    map(([wallet]) => {
      ...
      if (wallet && wallet.provider) {
        ...
        if (chain && chain.id && supportedChainIds.includes(parseInt(chain.id))) {
          ...
          return { ...resetOnboardState, connected: true, chain, icon: _icon, label: _label };
        }
      } else {
    ...
    catchError((err) => {
      reportError({ ...err, msgId: 'global.error.unsupportedChain', code: 500 });
