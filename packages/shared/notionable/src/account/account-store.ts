...
import { BehaviorSubject, distinctUntilChanged, map, Observable, scan, Subject } from 'rxjs';
import { BalanceSummary, AssetSummary, NOTESummary } from '@notional-finance/sdk-v2/account';
  ...
  return (
    ...
    Array.from(prev.keys()).every((key: T) => {
      if (prev.has(key) && curr.has(key)) {
        return compareHashKey(prev.get(key) as Hashable, curr.get(key) as Hashable);
      }
      return false;
    })
  );
  ...
  return accountState$.pipe(
    distinctUntilChanged((a, b) => {
      switch (key) {
        ...
        case 'noteSummary':
          return a[key] !== null && b[key] !== null && compareHashKey(a[key]!, b[key]!);
        default:
...
export const account$ = selectAccountState('account') as Observable<Account | null>;
export const balanceSummary$ = selectAccountState('balanceSummary') as Observable<
  Map<number, BalanceSummary>
>;
export const assetSummary$ = selectAccountState('assetSummary') as Observable<
  Map<string, AssetSummary>
>;
export const noteSummary$ = selectAccountState('noteSummary') as Observable<NOTESummary | null>;
export const accountConnected$ = selectAccountState('accountConnected') as Observable<boolean>;
export const isReadOnly$ = selectAccountState('isReadOnly') as Observable<boolean>;
export const readOnlyAddress$ = selectAccountState('readOnlyAddress') as Observable<
  string | undefined
>;
