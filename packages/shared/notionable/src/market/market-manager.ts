...
function getAllMarkets(source: Observable<System | null>): Observable<Map<number, CurrencyMarket>> {
  return source.pipe(
    mergeMap((system) => {
      if (system) {
        return forkJoin(
          system.getTradableCurrencies().map(({ id, assetSymbol, underlyingSymbol }) =>
            of({
              id,
              symbol: assetSymbol,
              underlyingSymbol,
              markets: new Map(system.getMarkets(id).map((market) => [market.marketKey, market])),
...
function areMarketsEqual(prev: Map<number, CurrencyMarket>, curr: Map<number, CurrencyMarket>) {
