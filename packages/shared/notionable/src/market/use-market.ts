...
export const useSelectedMarket = (selectedMarketKey: string | null) => {
  ...
  return currencyMarkets.get(Number(currencyId))?.markets.get(selectedMarketKey);
};
  ...
  return markets.map((m, index) => {
    ...
    if (underlyingAmount) {
      try {
        ...
        tradeRate = Market.formatInterestRate(m.interestRate(fCashAmount, underlyingAmount));
      } catch {
