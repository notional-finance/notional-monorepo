...
export const additionalTokens$ = system$.pipe(
  ...
  switchMap((system) => {
    return of([
      { symbol: 'NOTE', spender: system!.getStakedNote().address, contract: system!.getNOTE() },
      { symbol: 'WETH', spender: system!.getStakedNote().address, contract: system!.getWETH() },
      { symbol: 'sNOTE', spender: AddressZero, contract: system!.getStakedNote() },
    ] as ERC20Token[]);
  })
);
  ...
  map(([currencies, system]) => {
    if (system) {
      return currencies.flatMap((currency) => {
        const { assetSymbol, id, assetContract, underlyingContract, underlyingSymbol } = currency;
...
const _allTokens$ = combineLatest(systemTokens$, additionalTokens$).pipe(
  map(([systemTokens, additionalTokens]) => [...systemTokens, ...additionalTokens])
);

system$.subscribe((system) => {
  if (system) {
    const currencies = new Map(system!.getAllCurrencies().map((c) => [c.id, c]));
    const tradableCurrencies = new Map(system!.getTradableCurrencies().map((c) => [c.id, c]));
...
_allTokens$.subscribe((tokens) => {
  updateCurrencyState({ tokens: new Map(tokens.map((token) => [token.symbol, token])) });
});

systemTokens$.subscribe((systemTokens) => {
  updateCurrencyState({systemTokens: new Map(systemTokens.map((systemToken) => [systemToken.symbol, systemToken]))})
})