...
_updateSubject
  .pipe(scan((state, update) => ({ ...state, ...update }), initialCurrencyState))
  .subscribe(_currencyStore);

export const currencyState$ = _currencyStore.asObservable().pipe(shareReplay(1));
...
export const tokens$ = selectCurrencyState('tokens') as Observable<Map<string, ERC20Token>>;
export const currencies$ = selectCurrencyState('currencies') as Observable<Map<number, Currency>>;
export const tradableCurrencies$ = selectCurrencyState('tradableCurrencies') as Observable<
  Map<number, Currency>
>;
