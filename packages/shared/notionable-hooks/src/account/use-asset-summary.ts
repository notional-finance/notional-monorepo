...
import { convertRateToFloat, LEND_BORROW, PORTFOLIO_ACTIONS } from '@notional-finance/utils';
...
export function useAssetSummary(borrowOrLend: LEND_BORROW) {
  ...
  const assetSummary: AssetSummary[] = Array.from(assetSummaries?.values() ?? [])
    .filter(
      (a) =>
        a.liquidityToken === undefined &&
        ((borrowOrLend === LEND_BORROW.BORROW && a.fCash?.notional.isNegative()) ||
        ...
        .map(({ tradeRate, market }) => {
          ...
          return {
            ...
            rollMaturityRoute: `${PORTFOLIO_ACTIONS.ROLL_MATURITY}?assetKey=${
              a.assetKey
            }&marketKey=${market.marketKey}${isPartialRoll ? '&partialRoll=true' : ''}`,
          };
        });
